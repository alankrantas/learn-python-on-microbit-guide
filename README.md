...
之後我們會再談到，怎麼預防或是事後攔截這種有可能在執行時發生的錯誤（雖然說錯誤要怎麼解決，通常取決於錯誤本身是什麼）。這裡只是先讓你知道，型別轉換不一定會成功。
...
可以發現浮點數的小數點被截掉了。但有時候我們可以故意用這個方法來去掉小數位，特別是四捨五入（之後會提）有可能會讓整數超過我們需要的範圍的時候。

## Python 基礎：for 迴圈與集合迭代
