...
> 在其他程式語言中會有陣列（array），但陣列是長度固定、也只能儲存同一種資料型別的集合。Python 不是沒有陣列，但它最主要和最常用的集合就是串列。
...
既然 Python 變數是路牌，當你把它指向新的值時，就不再會指向串列元素了。所以若你想修改原始串列的值，直接改 item 變數是沒用的。
...
range(n) 函式傳回的東西不是串列，而是個型別為 range 的集合，內容為 0 到 n-1，剛好對應到有 n 個元素的串列的索引。

你可以看到，我們必須用 list() 把它轉換成串列才看得到內容，但 for 迴圈是可以直接迭代 range 集合的。所以你可以把程式改寫成如下：
...
這麼一來不管 lst 實際上有多少元素，for 迴圈都能正確無誤處理到所有元素了。
...
這裡也可以看到，串列內容是可以照逗點分行寫的（怎麼縮排則無所謂），連最後一個元素後面都可以加上逗號。在某些時候，這麼寫有助於增進程式的閱讀性。
